...
def selects_most_prominent_color_and_returns_with_count
  'SELECT color, COUNT(0) As color_count FROM bears GROUP BY color ORDER BY color_count DESC LIMIT 1;'
end

def counts_number_of_bears_with_goofy_temperaments
  'SELECT COUNT(0) As goofy_bears FROM bears WHERE temperament = "goofy";'
end
